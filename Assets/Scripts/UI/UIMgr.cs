...
public class UIMgr : MonoSingleton<UIMgr>
{
    ...
    {
        if (OperationPanel.IsActive()) return;
        ...
        ThinkingIcon.DOAnchorPos(new Vector2(_thinkIconPos.x, _thinkIconPos.y), duration).SetEase(GetRandomEase());
    ...
    public void OperationPanelFadeOut(CanvasGroup canvasGroup, RectTransform rectTransform, float xPos, float yPos = 0f, float duration = 0.75f)
    {
        ...
        ThinkingIcon.DOAnchorPos(new Vector2(_thinkIconPos.x, -1000f), duration).SetEase(GetRandomEase());
        PanelJumpFadeOut(canvasGroup, rectTransform, xPos, yPos, duration);

        GameObject[] gos = { canvasGroup.gameObject, OperationPanel.gameObject };
